...
def charge_bank():
    ...
    with tracer.start_as_current_span("Request to Charge API", kind=trace.SpanKind.CLIENT) as span:
        ...
        headers = {}
        inject(headers)
        span.add_event("Sending Request")                       # Adding an event to the span, it allows to co relate certain logs.
        resp = requests.get(url, headers=headers)
        span.add_event("Reuest Sent", attributes={"url":url})
        span.set_attribute("HTTP_STATUS_CODE", resp.status_code)   # adding extra information about certain events that occur in our application
